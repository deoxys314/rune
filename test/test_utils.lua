...
-- local lust = require('test.lust').nocolor()
...
lust.paths.raw_eq = {
    test = function(first, second)
        ...
        return result,
               'exepcted rawequal(' .. first_str .. ', ' .. second_str ..
                   ') to be true', 'expected rawequal(' .. first_str .. ', ' ..
                   second_str .. ') to be false'
    end,
}
...
describe('Test Utilities', function()
    ...
    describe('Array', function()
        ...
        describe('methods', function()
            ...
            it('foreach', function()
                expect(function()
                    Array {true, false, true}:foreach(function(v)
                        return not v
                    end)
                end).to_not.fail()
                do
            ...
            it('initialize', function()
                expect(Array.initialize(3, function(x)
                    return x * x
                end)).to.equal(Array {1, 4, 9})
            end)
            it('len', function()
            ...
            it('map', function()
                expect(Array.range(3):map(function(x)
                    return '$' .. x
                end)).to.equal(Array {'$1', '$2', '$3'})
                do
                    local const = '@'
                    expect(Array.range(4):map(function(n)
                        return const .. n
                    end)).to.be(Array {'@1', '@2', '@3', '@4'})
                end
            end)
            it('pop', function()
    ...
    describe('HashMap', function()
        ...
        describe('metamethods', function()
            ...
            it('__add', function()
                ...
                expect(HashMap {f = 15} + HashMap {g = 15}).to.be(HashMap {
                    g = 15,
                    f = 15,
                })
            end)
        ...
        describe('methods', function()
            it('add', function()
                ...
                expect(HashMap {a = 3}:add('b', 5)).to
                    .be(HashMap {a = 3, b = 5})
            end)
            it('update', function()
                expect(HashMap():update({a = 1, b = 2})).to.be(HashMap {
                    a = 1,
                    b = 2,
                })
            end)
            ...
            it('initialize', function()
                expect(HashMap.initialize(3, function(n)
                    return n, n * n
                end)).to.be(HashMap {1, 4, 9})
            end)
            ...
            it('filter', function()
                expect(HashMap {a = 1, b = 2, c = 3, d = 4}:filter(
                           function(_, v) return v % 2 == 0 end)).to.be(
                    HashMap {b = 2, d = 4})
            end)
            ...
            it('foreach', function()
                do
                    ...
                    expect(map:foreach(function(k, v)
    ...
    describe('Set', function()
        ...
        describe('metamethods', function()
            ...
            it('__add', function()
                ...
                expect(Set {5, 10} + Set {1, 2, 3, 4}).to.be(Set {
                    1, 2, 3, 4, 5, 10,
                })
            end)
            it('__sub', function()
                expect(Set {1, 2, 3} - Set {2}).to.be(Set {1, 3})
                expect(Set {1, 2, 3} - Set {true, false}).to.be(Set {1, 2, 3})
            end)
            it('__tostring',
               function()
        ...
        describe('initialization', function()
            it('array_like', function()
                expect(Set {'a', 'b', 'c'}).to.be(Set {'c', 'b', 'a'})
            end)
            it('hash_like', function()
                expect(Set {a = 1, b = 1, c = 1}).to.be(Set {'a', 'b', 'c'})
            end)
            it('mixed',
               function()
                expect(Set {'a', b = 3}).to.be(Set {'a', 'b'})
            end)
            it('negative_keys', function()
                expect(Set {[-1] = true, [-5] = true, [0] = true}).to.be(Set {
                    -1, 0, -5,
                })
            end)
            it('float_keys', function()
                expect(Set {[0.3] = 'junk', [100.005] = 'junk'}).to.equal(Set {
                    100.005, 0.3,
                })
            end)
        end)
        ...
        describe('methods', function()
            ...
            it('difference', function()
                ...
                expect(Set {1, 2, 3, 4}:difference(Set {1, 2})).to
                    .be(Set {3, 4})
            end)
            it('intersection', function()
                expect(Set {1, 2, 3}:intersection(Set {3, 4, 5})).to.be(Set {3})
                expect(Set {15, 30, 45}:intersection(Set {7, 14, 21})).to.be(
                    Set {})
                expect(Set {1, 2, 3}:intersection(Set {2, 3, 4})).to.be(Set {
                    2, 3,
                })
            end)
            it('items', function()
    ...
    describe('MultiSet', function()
        ...
        describe('methods', function()
            it('test', function()
                expect(nil).to.equal('Not Implemented')
            end)
        end)
    end)
    ...
    describe('stringx', function()
        ...
        it('split', function()
            expect(collect(stringx.split('testing string'))).to.equal({
                'testing', 'string',
            })
        end)
    ...
    describe('pprint', function()
        ...
        it('table', function()
            expect(pprint({'a', 'b', 'c'})).to.equal(
                '{\n  [1] = "a",\n  [2] = "b",\n  [3] = "c",\n}')
        end)
        it('nested_table', function()
            expect(pprint({{}, {}})).to.equal('{\n  [1] = {},\n  [2] = {},\n}')
        end)
        ...
        it('dump', function()
            lust.before(function()
                local null = io.open('/dev/null', 'w')
                io.output(null)
            end)
            lust.after(function() io.close() end)

            expect(function() pprint.dump({1, 2, 3}) end).to_not.fail()
        end)
    ...
    describe('iterx', function()
        ...
        it('collect', function()
            ...
            expect(iterx.collect(iter)).to.equal({
                '1', '2', '3', '4', '5', '6', '7', '8',
            })
        end)

        it('take', function()
            local iter = iterx.take(string.gmatch(alphabet, '.'), 5)
            expect(iterx.collect(iter)).to.equal({'a', 'b', 'c', 'd', 'e'})
        end)
    end)
end)
